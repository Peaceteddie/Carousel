...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateInputValue(input: HTMLInputElement, value: HTMLElement | null, newValue: number) {
    ...
    function setupRealTimeUpdates() {
        const inputs = [rateLimitSlider, maxDepthInput, minImageSizeInput, maxConcurrentFetchesInput, similarityScoreInput];
            ...
            if (activeTab?.id) {
                chrome.tabs.sendMessage(activeTab.id, { action: 'executeMainAndProcess' }, (response) => {
                    if (chrome.runtime.lastError) {
                        ...
                        showToast("error", `Error: ${chrome.runtime.lastError.message}`);
                    } else if (response) {
                        ...
                        showToast("error", "No response received. The process may still be running in the background.");
                    }
                });
            } else {
                console.error("No active tab found. Please make sure you have an active tab open.");
                showToast("error", "No active tab found. Please make sure you have an active tab open.");
            }
        });
    }
        ...
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
            ...
            if (activeTab?.id) {
                chrome.tabs.sendMessage(activeTab.id, { action: 'cancelProcessingImages' }, (response) => {
                    if (chrome.runtime.lastError) {
                        console.error("Error cancelling process:", chrome.runtime.lastError);
                        showToast("error", `Error: ${chrome.runtime.lastError.message}`);
                    } else if (response && response.status === 'Success') {
                        console.log("Processing cancelled successfully");
                        showToast("success", "Image processing cancelled.");
                    } else {
                        console.error("Unexpected response when cancelling:", response);
                        showToast("error", "Unexpected error when cancelling processing.");
                    }
                });
            } else {
                console.error("No active tab found. Unable to cancel processing.");
                showToast("error", "No active tab found. Unable to cancel processing.");
            }
        });
    }
